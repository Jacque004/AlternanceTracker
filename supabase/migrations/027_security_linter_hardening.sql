-- Durcissement linter Supabase — idempotent (rejouable si une fonction manque ou a déjà été déplacée)
--
-- Non couvert en SQL : Dashboard Auth → Attack Protection → Leaked password protection

-- ---------------------------------------------------------------------------
-- 1) search_path figé uniquement si la fonction existe
-- ---------------------------------------------------------------------------
DO $$
DECLARE
  fn text;
...
-- ---------------------------------------------------------------------------
-- 2) Triggers SECURITY DEFINER dans private (créés ici, pas un ALTER du public)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 4) EXECUTE : PUBLIC / anon retirés si la fonction existe
-- ---------------------------------------------------------------------------
DO $$
DECLARE
  rec record;
...
EXCEPTION
  WHEN OTHERS THEN
    RAISE NOTICE 'Déplacement de pg_net ignoré : %', SQLERRM;
END $$;
